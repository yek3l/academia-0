...
class Place(object):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                if self.ant.container and not self.ant.ant:
                    self.ant.ant = insect
                elif insect.container and not insect.ant:
                    insect.ant = self.ant
                    self.ant = insect
                # END Problem 9
        else:
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if insect.is_queen:
                return None 
                # bees_win() # is there anything else we could put here??? LOOK AT THIS LATER OMG
            # END Problem 13
...
class Ant(Insect):
    ...
    container = False
    is_queen = False
    dmg_doubled = False # keeps track of if the queen ant has doubled this ant's damange bc we weren't a huge fan of the list plan
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_range != self.min_range:
            location,current_range = location.entrance, current_range + 1
        while current_range != self.max_range and location != hive:
            if location.bees:
...
class Water(Place):
    ...
    def add_insect(self, insect):
        ...
        if not insect.watersafe:
            Insect.reduce_armor(insect, insect.armor) #why doesn't putting self.ant.armor instead of insect.armor work?
        # END Problem 11
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.place == None:
            bees_in_place = list(ant_place.bees)
...
class ShortThrower(ThrowerAnt):
    ...
    name = 'Short'
    food_cost = 2
    max_range = 4 #for zero-indexed counting
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUI
    # END Problem 4
...
# BEGIN Problem 8
class WallAnt(Ant):
    """ An ant that doesn't do anything each turn but has hella armor."""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class HungryAnt(Ant):
    ...
    name = 'Hungry'
    food_cost = 4
    time_to_digest = 3 #number turns to comsume
    # BEGIN Problem 6
    implemented = True   # Change to True to view in the GUI
    # END Problem 6
    ...
    def action(self, colony):
        ...
        if self.digesting != 0:
            ...
            self.eat_bee(random_or_none(self.place.bees)) #how do we get this bee?
        # END Problem 6
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if not self.is_queen: #for imposter queen
            ...
            while location != None:
                if location.ant:
                    if not location.ant.dmg_doubled: # not in self.doubled_ants:
                        location.ant.damage *= 2
                        location.ant.dmg_doubled = True
                    # self.doubled_ants.append(location.ant)
                    if location.ant.container and location.ant.ant and not location.ant.ant.dmg_doubled:
                        location.ant.ant.damage *= 2
                        location.ant.ant.dmg_doubled = True
