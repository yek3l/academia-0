...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 2
            src.remove_front() #to get rid of the dot
            curr_val = scheme_read(src) #because we want the whole exp, not just the next single item; also makes this a pair
            if src.current() == ')':
                src.remove_front()
                return curr_val
            else:
                raise SyntaxError('dotted pair has too many things after the dot!')
            # END PROBLEM 2
        else:
            # BEGIN PROBLEM 1
            next_exp = scheme_read(src) #scheme_read gets rid of first element!!
            exp_rest = read_tail(src)
            return Pair(next_exp, exp_rest)
            # END PROBLEM 1
    except EOFError:
        raise SyntaxError('unexpected end of file')
