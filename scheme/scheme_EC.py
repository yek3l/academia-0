...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda operand: scheme_eval(operand, env)) # needed lambda bc we scheme_eval takes two args but rest is only one
...
def scheme_apply(procedure, args, env):
    ...
    if isinstance(procedure, PrimitiveProcedure):
        ...
        return eval_all(procedure.body, new_env) #MORE SPIDERS!!
...
def eval_all(expressions, env):
    ...
    while expressions is not nil:
        ...
        exp_value = scheme_eval(exp, env, True) #WHY
...
class Frame:
    ...
    def __repr__(self):
        if self.parent is None:
            return '<Global Frame>'
        s = sorted(['{0}: {1}'.format(k, v) for k, v in self.bindings.items()]) #k, v == key, value
        return '<{{{0}}} -> {1}>'.format(', '.join(s), repr(self.parent)) #this whole thing basically represents the whole dict s and then shows it has the parent, it basically shows all the stuff in the frame but doesn't name the frame?
    ...
    def lookup(self, symbol):
        ...
        frame = self.parent
        while frame != None: #and the reason this is frame and not frame.parent is bc ????????????
    ...
    def make_child_frame(self, formals, vals):
        ...
        def length(lst):
            if lst == nil:
                return 0
            return 1 + length(lst.second)
        if length(formals) != length(vals):
            raise SchemeError(('number of argument values {0} does not match number of formal parameters {1}!').format(vals, formals))
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:
            return self.fn(*python_args) # hOLY SHIT THE * JUST MEANS 'TAKE HOWEVER FUCKING MANY ARGUMENTS U NEED FROM THE LIST WHOSE NAME FOLLOWS THIS ASTERISK GODDAMN WOW THIS IS POWERFUL'
        except:
            raise SchemeError(('The function {0} has the wrong number of arguments: {1}').format(self.fn, python_args))
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 6
        env.define(target, scheme_eval(expressions.second.first, env)) # expressions.second.first because expressions.second is Pair(B, nil)
        return target
        # END PROBLEM 6
    elif isinstance(target, Pair) and scheme_symbolp(target.first):
        # BEGIN PROBLEM 10
        env.bindings[target.first] = LambdaProcedure(target.second, expressions.second, env)
        expressions.second.map(lambda operand: scheme_eval(operand, env))
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        if bindings.first.second.second is not nil:
            raise SchemeError(('Too many assignments, {0}, to variable {1}').format(bindings.first.second, bindings.first.first))
...
def do_mu_form(expressions, env): #why don't we ever refer back to this env ever again?
    """Evaluate a mu form."""
    check_form(expressions, 2)
    formals = expressions.first
    check_formals(formals)
...
def complete_apply(procedure, args, env):
    ...
    if isinstance(val, Thunk):
        return scheme_eval(val.expr, val.env, True) #SPIDERS!!
    else:
        return val
